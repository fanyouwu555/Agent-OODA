...
// 加载 storage 模块
const storagePath = path.join(__dirname, '../packages/storage/dist/index.js');
...
async function testMemorySystem() {
  ...
  try {
    const storageInstance = await storage.createStorage(dbPath);
    console.log('   ✓ 存储初始化成功\n');
    
    console.log(`2. 初始化记忆系统...`);
    const enableEmbedding = false; // 测试时禁用向量嵌入
    core.initializeMemorySystem(storageInstance.memories, enableEmbedding);
    console.log(`   ✓ 记忆系统初始化成功 (向量嵌入: ${enableEmbedding})\n`);
    
    console.log(`3. 加载预置角色...`);
    const personaManager = core.initializePersonaManager(storageInstance.memories);
    await personaManager.loadPersona('assistant');
    console.log('   ✓ 预置角色加载成功\n');
    
    console.log(`4. 测试记忆存储...`);
    const sessionMemory = core.getSessionMemory('test-session');
