...
export const timerTool = {
    ...
    permissions: [],
    async execute(input) {
        const timers = global.__timers || (global.__timers = new Map());
...
export const base64Tool = {
    ...
    async execute(input) {
        ...
        try {
            ...
            throw new Error(`Base64 ${action === 'encode' ? '编码' : '解码'}失败: ${error.message}`);
        }
    }
};
export const hashTool = {
    ...
    async execute(input) {
        ...
        try {
            ...
            hashBuffer = await crypto.subtle.digest(algorithm.toUpperCase().replace('SHA', 'SHA-'), data);
